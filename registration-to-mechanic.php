...
if (!empty($_POST)) {
	...
	if (isset($_POST['service-completion'])) {
		$service_completion = htmlspecialchars($_POST['service-completion']);
		$_SESSION['selected_service'] = $service_completion;
	}
	...
	if (isset($_POST['service-time'])) {
		$service_time = htmlspecialchars($_POST['service-time']);
		$date_time = $_SESSION['selected_date']  . ' ' . $service_time . ':00';
		$mechanic = $_SESSION['selected_mechanic'];
		$service_completion = $_SESSION['selected_service'];
		...
		if (isValidDateTime($date_time)) {
			...
			$stmt->bind_param("ii", $service_completion, $mechanic);
			$stmt->execute();
			$result = $stmt->get_result();
	...
	if (!isset($_POST['mechanic']) && !isset($_SESSION['selected_mechanic']) && !isset($_SESSION['selected_service']) && !isset($_SESSION['selected_date'])) {
		$message = "Nepasirinktas mechanikas.";
	}
	else if (!isset($_POST['mechanic']) && !isset($_POST['service-completion']) && isset($_SESSION['selected_mechanic']) && !isset($_SESSION['selected_service']) && !isset($_SESSION['selected_date'])) {
		$message = "Nepasirinkta paslauga.";
	}
	else if (!isset($_POST['service-completion']) && !isset($_POST['service-date']) && isset($_SESSION['selected_mechanic']) && isset($_SESSION['selected_service']) && !isset($_SESSION['selected_date'])) {
	...
	<div class="content">
		<?php if (!$form_submitted): ?>
			<form method='post' class="inputform">
				<?php if (!isset($mechanic) && !isset($service_completion) && !isset($service_date)): ?>
				<div class="input">
					<label for="mechanic">Registracija pas mechaniką</label>
					<select name="mechanic" id="mechanic">
						<option value="" disabled selected>Pasirinkite mechaniką iš sąrašo</option>
						<?php
						$query = "SELECT * FROM SYSTEM_USER WHERE fk_role=2 ORDER BY name, surname ASC";
						$result = mysqli_query($dbc, $query);
						...
						if ($result && mysqli_num_rows($result) > 0) {
							...
							echo '<option disabled>Sistemoje jokių meistrų nėra</option>';
						}
						?>
					</select>
				</div>

			<?php elseif (isset($mechanic) && !isset($service_completion) && !isset($service_date)): ?>
			<div class="input">
				<label for="service-completion">Paslauga</label>
				<select name="service-completion" id="service-completion">
					<option value="" disabled selected>Pasirinkite paslaugą iš sąrašo</option>
					<?php
					$mechanic_id = htmlspecialchars($_POST['mechanic']);

					$query = "SELECT name, duration_in_minutes, SERVICE_COMPLETION.id AS id FROM SERVICE_COMPLETION INNER JOIN SERVICE ON SERVICE_COMPLETION.fk_service = SERVICE.id WHERE fk_mechanic='$mechanic_id' AND removed = 0";
					$result = mysqli_query($dbc, $query);

					if ($result && mysqli_num_rows($result) > 0) {
						while ($row = mysqli_fetch_assoc($result)) {
							$hours = floor($row['duration_in_minutes'] / 60);
							$minutes = $row['duration_in_minutes'] % 60;

							echo '<option value="' . $row['id'] . '">' 
							. $row['name'] . ' (trukmė: ' . $hours. 'h ' . $minutes . 'min)' .
							'</option>';
						}
					} else {
						echo '<option disabled>Sistemoje jokių paslaugų, kurias atlieka meistras, nėra</option>';
					}
					?>
				</select>
			</div>
				...
				for ($i = 0; $i < 30; $i++) {
					...
					while ($row = mysqli_fetch_assoc($schedule)) {
						if ((int)$row['week_day'] == (int)$weekday) {
							...
							$query = "SELECT duration_in_minutes FROM SERVICE_COMPLETION WHERE id='$service_completion'";
...
<?php else: ?>
	<div class="inputform">
		...
		<a href="registration-to-mechanic.php" class="submit-button">Registruotis dar kartą</a>
	</div>
<?php endif; ?>
</div>
